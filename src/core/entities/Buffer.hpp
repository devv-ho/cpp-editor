...
class Buffer {
 public:
  ...
  [[nodiscard]] std::expected<std::string_view, BufferError> line(std::size_t idx) const noexcept {
    if (idx >= lines_.size()) return std::unexpected(BufferError::LineOutOfRange);
    return lines_[idx];
  }
  ...
  [[nodiscard]] std::expected<void, BufferError> insert_char(Position pos, char ch) {
    if (pos.line >= lines_.size()) return std::unexpected(BufferError::LineOutOfRange);
    auto &ln = lines_[pos.line];
    if (pos.col > ln.size()) return std::unexpected(BufferError::ColOutOfRange);
  ...
  [[nodiscard]] std::expected<void, BufferError> join_with_prev(Position pos) {
    if (pos.line == 0) return std::unexpected(BufferError::LineOutOfRange);
    if (pos.line >= lines_.size()) return std::unexpected(BufferError::LineOutOfRange);
